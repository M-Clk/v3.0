...
namespace Otomasyon
{
    public partial class frmConfiguration : Form
    {
        ...
        private void IsletmeAdiGetir()
        {
            ...
            {
                var IsletmeOku = _dbOperations.SqlTextReader("SELECT Adi,Telefon,Email,Adres FROM Isletme");
                if (IsletmeOku.Read())
                {
            ...
            {
                MessageBox.Show(
                    "Veritabanına bağlantı yapılamadı. Lütfen veritabanı yapılandırmasını doğru yaptığınızdan emin olun.",
                    " Bağlantı Sorunu", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }

        private string SeriNoAl()
        {
        ...
        private void CBDoldur(ComboBox cb, string procString)
        {
        ...
        private string MClkSifremele(string sifre)
        {
            ...
            mclksiz = md5li;
            var sonHal =
                new char[24]; // "MCLKMXXXXCXXXXLXXXXKXXXX"; Kendi şifreleme yöntemim => MCLK-MXXXX-CXXXX-LXXXX-KXXXX  (24)
            sonHal[0] = 'M';
            sonHal[1] = 'C';
            sonHal[2] = 'L';
            sonHal[3] = 'K';
        ...
        private void IlkKullaniciKontrol()
        {
            if (DbOperations.Connection.IsAvailable())
            {
                var dr = _dbOperations.OkuProcedure("KULLANICILARIGETIR", new SqlParameter[0]);
                if (dr.Read())
                {
        ...
        private void AktiflikKontrol(bool durum)
        {
            if (durum)
            {
                ...
                lblBilgi.ForeColor = Color.LimeGreen;
        ...
        private bool MailGonder()
        {
            ...
            smtp.Credentials = new NetworkCredential("mclk.yzlm@gmail.com", pass);
            smtp.Port = 587;
        ...
        private void ChangeGroupsVisible(bool status)
        {
        ...
        private void LoadControls()
        {
            try
            {
                ...
                foreach (string yazici in PrinterSettings.InstalledPrinters) cbYazici.Items.Add(yazici);
                var yaziciAdi = "";
                try
                {
        ...
        private void btnUygula_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Veritabanı ayarlarınız yapılandırıldı. Ancak veritabanı ile bağlantı kurulamadı. Bu halde hiçbir işlem yapamazsınız. ",
                    "Uyarı Mesajı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            if (DbOperations.Connection.IsAvailable())
            {
                _dbOperations.OkuScalar(
                    "Delete from Isletme; Insert Into Isletme Values ('" + txtIsletmeAdi.Text + "','" +
                    txtTelefon.Text + "','" + txtEmail.Text + "','" + txtAdres.Text + "')", CommandType.Text,
                    new SqlParameter[0]);
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            if (DbOperations.Connection.IsAvailable())
            {
                ...
                {
                    var EkleParam = new SqlParameter[3];
                    ...
                    {
                        nfBasarili.BalloonTipText = "İlk kullanıcı başarılı bir şekilde eklendi. " +
                                                    txtKullaniciAdi.Text +
                                                    " kullanıcı adı ile sisteme giriş yapabilirsiniz.";
                        nfBasarili.Visible = true;
                        nfBasarili.ShowBalloonTip(2000);
                    }
                }
                ...
                {
                    MessageBox.Show("Kullanıcı adınız en az 3, şifreniz de en az 6 karakter olmalıdır.",
                        "Karakter Sayısı Az", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
        ...
        private void btnBirimSil_Click(object sender, EventArgs e)
        {
            if (DbOperations.Connection.IsAvailable())
            {
                ...
                {
                    var silmeOnayi = MessageBox.Show("Seçili birimi silmek üzeresiniz. \nDevam etmek istiyor musunuz?",
                        "Silgi Onayı", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);

                    if (silmeOnayi == DialogResult.Yes)
                        try
                        {
                            Convert.ToInt32(_dbOperations.OkuScalar(
                                "Delete from Birimler Where Id=" + cbBirimler.SelectedValue, CommandType.Text,
                                new SqlParameter[0]));
                            CBDoldur(cbBirimler, "BIRIMSORGULAMA");
                        ...
                        {
                            MessageBox.Show(
                                "Seçili birim bazı ürünler için kullanıldığından silinemiyor. Silebilmek için önce bu birimi kullanan bütün ürünleri silmelisiniz.",
                                "Başarısız İşlem", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                }
                else
                {
                    MessageBox.Show("En az 1 birim kalana kadar silebilirsiniz.", "Sınır Aşıldı", MessageBoxButtons.OK,
        ...
        private void btnBirimEkle_Click(object sender, EventArgs e)
        {
            if (DbOperations.Connection.IsAvailable())
                try
                {
                    for (var i = 0; i < cbBirimler.Items.Count; i++)
                        if (txtBirimEkle.Text.ToUpper().Trim() == cbBirimler.Items[i].ToString().ToUpper().Trim())
                        {
                            MessageBox.Show("Eklemek istediğiniz birim zaten var.", "Uyarı", MessageBoxButtons.OK,
                                MessageBoxIcon.Warning);
                            return;
                        }

                    Convert.ToInt32(_dbOperations.OkuScalar(
                        "Insert Into Birimler(Adi,Kisaltma) Values('" + txtBirimEkle.Text.ToUpper() + "','" +
                        txtBirimKisaltma.Text.ToUpper() + "')", CommandType.Text, new SqlParameter[0]));
                    CBDoldur(cbBirimler, "BIRIMSORGULAMA");
                    txtBirimEkle.Text = "";
                ...
                catch (Exception ex)
                {
                    txtBirimEkle.Text = "";
                    txtBirimKisaltma.Text = "";
                    MessageBox.Show(ex + "Hatalı veri girişi yapıldı. Lütfen desteklenmeyen karakterleri girmeyin.",
                        "Başarısız İşlem", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            else
                MessageBox.Show(
        ...
        {
            if (txtBirimEkle.Text.Length >= 3 && txtBirimKisaltma.Text.Trim().Length <= 3 &&
                txtBirimKisaltma.Text.Trim().Length > 0)
        ...
        private string EksileriYokEt()
        {
            var mskstring = mskLisansAnahtari.Text.ToUpper();
        ...
        private void btnEtkinlestir_Click(object sender, EventArgs e)
        {
            ...
            if (LisansKontrol())
            {
                nfBasarili.BalloonTipText =
                    "Lisans anahtarınız kabul edildi! Barkodlu sistemi hayırlı günlerde kullanmanız dileğiyle...";
            ...
            {
                mskLisansAnahtari.Text = "";
                MessageBox.Show("Lisans anahtarı onaylanmadı. Lütfen tekrar deneyin.", "Lisans Onaylanmadı",
        ...
        private void txtBirimEkle_Enter(object sender, EventArgs e)
        {
            ...
            {
                txtBirimEkle.Text = "";
                txtBirimEkle.ForeColor = Color.Black;
            }
        }
        ...
        private void textBox1_Enter(object sender, EventArgs e)
        {
            ...
            {
                txtBirimKisaltma.Text = "";
                txtBirimKisaltma.ForeColor = Color.Black;
            }
        }
        ...
        private void btnLisansIste_Click(object sender, EventArgs e)
        {
            if (MailGonder())
            {
                MessageBox.Show("Lisans anahtarı talebiniz iletildi. Geri dönüt bekleyin.", "İşlem Başarılı",
            ...
            {
                MessageBox.Show(
                    "Bir sorun oluştu lütfen internet bağlantınız olduğundan emin olun ve tekrar deneyin. Ya da \"mclk.yzlm@gmail.com\" e-posta adresi aracılığı ile lisans anahtarı talep edin.",
                    "Başarısız İşlem", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void txtBirimEkle_Leave(object sender, EventArgs e)
        {
            ...
            {
                txtBirimEkle.ForeColor = Color.DarkGray;
        ...
        private void txtBirimKisaltma_Leave(object sender, EventArgs e)
        {
            ...
            {
                txtBirimKisaltma.ForeColor = Color.DarkGray;
        ...
        {
            if (e.Control && e.KeyCode == Keys.M) txtHidden.Focus();
        }
        ...
        private void btnVeritabaniOlustur_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("TÜm komutlar çaşıştırıldı.", "Veritabanı Başarıyla Oluştu", MessageBoxButtons.OK,
