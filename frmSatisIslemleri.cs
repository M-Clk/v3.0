...
namespace Otomasyon
{
    public partial class frmSatisIslemleri :Form
    {
        ...
        void SatislariGoster()
        {
            ...
            SqlDataReader FiltreOkuyucu = SqlCnnctn.OkuProcedure("SATISFILTRELE", FilterParameter);
            Thread.CurrentThread.CurrentCulture = new CultureInfo("tr-TR"); //Sorgu bittiğinde sonucu tekrar aynı formatta almak için türk formatına geri çevir.
            ...
            if(FiltreOkuyucu != null)
                while(FiltreOkuyucu.Read())
                {
                    ...
                    satir[0] = FiltreOkuyucu[0] == DBNull.Value ? "0" : Convert.ToInt32(FiltreOkuyucu[0]).ToString("D10"); //Satış No
                    ...
                    decimal Tutar = FiltreOkuyucu[4] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[4]);
            ...
            using(SqlDataReader DetayOku = SqlCnnctn.OkuProcedure("SATISDETAYIGETIR", DetayAl))
            {
                ...
                if(nakit == 0)
                {
                    ...
                    if(taksit == 0)
                        ...
                        lblParaUstu.Text = "Taksit : " + taksit.ToString() + " Ay";
                    lblKredi.Text = "";
                    lblTaksit.Text = "";
                }
                else
                {
                    ...
                    if(kredi > 0)
                    {
                        ...
                        if(taksit == 0)
                            ...
                            lblTaksit.Text = "Taksit : " + taksit.ToString() + " Ay";
                    }
                }
                    ...
                    for(var satir = 0; satir < dgSatislar.RowCount; satir++)
                    {
                        if(dgSatislar.Rows[satir].Visible)
                        {
                            if(kolon == 0)
                            {
                                ...
                                ws1.Cells[satir - yukari + 4, kolon + 1].Value = Convert.ToDecimal(dgSatislar.Rows[satir - yukari].Cells[kolon].Value);
                            else
                ...
                for(var kolon = 0; kolon < dgSatisDetayi.ColumnCount; kolon++)
                {
                    ...
                    for(var satir = 0; satir < dgSatisDetayi.RowCount; satir++)
                    {
                        ...
                        if(kolon == 0)
                        {
                            ...
                            ws1.Cells[satir + 3, kolon + 1].Value = Convert.ToDecimal(dgSatisDetayi.Rows[satir].Cells[kolon].Value);
                        else if(kolon < 6)
