...
namespace Otomasyon
{
    public partial class frmSatisIslemleri : Form
    {
        ...
        private decimal KasadakiSonPara, EkGelir, EkGider;
        private decimal netKazanc, topNakit, topKredi;
        private string seciliMusteriAdi = "", seciliKasiyerAdi = "";
        private int seciliSatis, seciliTaksit;
        private DateTime seciliTarih;
        private decimal seciliTutar, seciliNakit, seciliKredi;
        private readonly DbOperations SqlCnnctn = new DbOperations();
        ...
        public frmSatisIslemleri(int satisId, decimal tutar, DateTime tarih, string musteriAd, string kasiyerAdi,
            decimal nakit, decimal kredi, int taksit)
        {
            InitializeComponent();
            seciliSatis = satisId;
            seciliTutar = tutar;
        ...
        private void SatislariGoster()
        {
            ...
            topKredi = 0;
            topNakit = 0;
            var FilterParameter = new SqlParameter[5];
            ...
            var FiltreOkuyucu = SqlCnnctn.OkuProcedure("SATISFILTRELE", FilterParameter);
            Thread.CurrentThread.CurrentCulture =
                new CultureInfo(
            ...
            var satir = new string[dgSatislar.ColumnCount];
            if (FiltreOkuyucu != null)
                while (FiltreOkuyucu.Read())
                {
                    if (satir[0] == null)
                        KasadakiSonPara = FiltreOkuyucu[7] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[7]);
                    if (cbSecenek.SelectedIndex != 0)
                        sonSatirKasadaki = Convert.ToDecimal(FiltreOkuyucu[10]);
                    satir[0] = FiltreOkuyucu[0] == DBNull.Value
                        ? "0"
                        : Convert.ToInt32(FiltreOkuyucu[0]).ToString("D10"); //Satış No
                    ...
                    var Tutar = FiltreOkuyucu[4] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[4]);
                    topKar += FiltreOkuyucu[5] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[5]);
                    topTutar += Tutar;
                    topKredi += FiltreOkuyucu[9] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[9]);

                    dgSatislar.Rows.Add(satir);
                    if ((FiltreOkuyucu[4] == DBNull.Value ? 0 : Convert.ToDecimal(FiltreOkuyucu[4])) == 0)
                        dgSatislar.Rows[dgSatislar.RowCount - 1].Visible = false;
                }
            ...
            var iadeEdilenTop = Convert.ToDecimal(SqlCnnctn.bilgiMessage);
            satir[0] = null;
            var topMaliyet = topTutar - topKar;
            if (cbSecenek.SelectedIndex == 3)
                gecici = 0;
            else
                gecici = KasadakiSonPara - topMaliyet + iadeEdilenTop / 100;
            netKazanc = gecici;
            lblTopTutar.Text = "Toplam Satış Tutarı : " + topTutar + " ₺";
            lblKar.Text = "Toplam Satış Kârı : " + topKar + " ₺";
            lblNet.Text = "Kasadaki Toplam : " + KasadakiSonPara + " ₺";
            lblEkGelir.Text = "Toplam Ek Gelir : " + EkGelir + " ₺";
            lblEkGider.Text = "Toplam Gider : " + -EkGider + " ₺";
        ...
        private void GrafikteGoster(int Secenek)
        {
            foreach (var seriler in chrtSatislar.Series) seriler.Points.Clear();
            // DateTime tarh;
            for (var i = 0; i < dgSatislar.Rows.Count; i++)
                //if (Secenek == 0)
                //{
                //    tarh = Convert.ToDateTime(dgSatislar.Rows[i].Cells[4].Value);
                if (dgSatislar.Rows[i].Visible)
                {
                    chrtSatislar.Series[0].Points.AddXY(Convert.ToInt32(dgSatislar.Rows[i].Cells[0].Value),
                        Convert.ToDecimal(dgSatislar.Rows[i].Cells[4].Value));
                    chrtSatislar.Series[1].Points.AddXY(Convert.ToInt32(dgSatislar.Rows[i].Cells[0].Value),
                        Convert.ToDecimal(dgSatislar.Rows[i].Cells[5].Value));
                }
        ...
        private void SatisDetayiGoster(int Id, decimal tutar, DateTime tarih, string musteriAd, string kasiyerAdi,
            decimal nakit, decimal kredi, int taksit)
        {
            dgSatisDetayi.Rows.Clear();

            var DetayAl = new SqlParameter[1];
            DetayAl[0] = new SqlParameter();
            DetayAl[0].ParameterName = "@SatisId";
            DetayAl[0].SqlDbType = SqlDbType.Int;
            DetayAl[0].SqlValue = Id;

            using (var DetayOku = SqlCnnctn.OkuProcedure("SATISDETAYIGETIR", DetayAl))
            {
                var satir = new string[dgSatisDetayi.ColumnCount];
                while (DetayOku.Read())
                {
                    ...
                    var ondalik = Convert.ToInt32(DetayOku[4]);
                    var kesirli = Convert.ToDecimal(DetayOku[4]);
                    ...
                    if (satir[6] == "True" && Convert.ToDecimal(satir[4]) <= 0)
                    {
                        ...
                        dgSatisDetayi.Rows[dgSatisDetayi.Rows.Count - 1].DefaultCellStyle.SelectionBackColor =
                            Color.LightPink;
                    }
                }
                ...
                lblTarih.Text = "Satış Tarihi : " + tarih;
                ...
                if (nakit == 0)
                {
                    ...
                    if (taksit == 0)
                        lblParaUstu.Text = "Peşin Ödeme";
                    else
                        lblParaUstu.Text = "Taksit : " + taksit + " Ay";
                    lblKredi.Text = "";
                    lblTaksit.Text = "";
                }
                else
                {
                    ...
                    var paraUstu = nakit + kredi - tutar;
                    if (paraUstu > 0)
                        lblParaUstu.Text = "Para Üstü : " + paraUstu.ToString("F2") + " ₺";
                    else
                        lblParaUstu.Text = "";
                    if (kredi > 0)
                    {
                        ...
                        if (taksit == 0)
                            lblTaksit.Text = "Peşin Ödeme";
                        else
                            lblTaksit.Text = "Taksit : " + taksit + " Ay";
                    }
                }
        ...
        private void dgSatislar_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                SatisDetayiGoster(seciliSatis, seciliTutar, seciliTarih, seciliMusteriAdi, seciliKasiyerAdi,
                    seciliNakit, seciliKredi, seciliTaksit);
            }
        }

        private void dtBaslangicTarihi_ValueChanged(object sender, EventArgs e)
        {
            if (dtBaslangicTarihi.Visible)
                SatislariGoster();
        }

        private void dtBitisTarihi_ValueChanged(object sender, EventArgs e)
        {
            if (dtBitisTarihi.Visible)
                SatislariGoster();
        }

        private void btnExcelDetay_Click(object sender, EventArgs e)
        {
            if (dgSatisDetayi.RowCount > 0)
                DetayiExceleAktar();
        }
        //void DetayiYazdir()
        //{
        ...
        //    }
        //}
        private void ExceleAktar(string baslik)
        {
            ...
            if (dialogResult == DialogResult.OK)
            {
                ...
                var ws1 = _package.Workbook.Worksheets.Add("Satış Listesi");
                ...
                for (var i = 0; i < dgSatislar.ColumnCount - 4; i++)
                ...
                for (var kolon = 0; kolon < dgSatislar.ColumnCount - 4; kolon++)
                {
                    ...
                    for (var satir = 0; satir < dgSatislar.RowCount; satir++)
                        if (dgSatislar.Rows[satir].Visible)
                        {
                            if (kolon == 0)
                            {
                                ws1.Cells[satir - yukari + 4, kolon + 1].Value =
                                    Convert.ToInt32(dgSatislar.Rows[satir - yukari].Cells[kolon].Value);
                                if ((satir - yukari) % 2 == 0)
                                {
                                    ws1.Cells["A" + (satir - yukari + 4) + ":F" + (satir - yukari + 4)].Style.Fill
                                        .PatternType = ExcelFillStyle.Solid;
                                    ws1.Cells["A" + (satir - yukari + 4) + ":F" + (satir - yukari + 4)].Style.Fill
                                        .BackgroundColor.SetColor(acikGri);
                                }
                            }
                            else if (kolon > 3)
                            {
                                ws1.Cells[satir - yukari + 4, kolon + 1].Value =
                                    Convert.ToDecimal(dgSatislar.Rows[satir - yukari].Cells[kolon].Value);
                            }
                            else
                            {
                                ws1.Cells[satir - yukari + 4, kolon + 1].Value =
                                    dgSatislar.Rows[satir - yukari].Cells[kolon].Value;
                            }
                ...
                ws1.Cells["D" + (sonSatir + 1) + ":F" + (sonSatir + 1)].Style.Border.Bottom.Style =
        ...
        {
            saveExceleKaydet.Filter = "Excel Dosyaları (*.xlsx)|*.xlsx";
            saveExceleKaydet.FileName = "Satis_Detayi(Satis_No=" + seciliSatis + ")";
            var dialogResult = saveExceleKaydet.ShowDialog();
            if (dialogResult == DialogResult.OK)

            {
                ...
                _package = new ExcelPackage(new MemoryStream());
                var ws1 = _package.Workbook.Worksheets.Add(seciliSatis + " Numaralı Satış");
                ...
                for (var i = 0; i < dgSatisDetayi.ColumnCount; i++)
                    if (i < 6)
                    ...
                    for (var satir = 0; satir < dgSatisDetayi.RowCount; satir++)
                    {
                        if (kolon == 0)
                        {
                            ws1.Cells[satir + 3, kolon + 1].Value =
                                Convert.ToInt32(dgSatisDetayi.Rows[satir].Cells[kolon].Value);
                            if (satir % 2 == 0)
                            {
                        ...
                        {
                            ws1.Cells[satir + 3, kolon + 1].Value =
                                Convert.ToDecimal(dgSatisDetayi.Rows[satir].Cells[kolon].Value);
                        }
                        else if (kolon < 6)
                        {
        ...
        {
            var UrunSatisVeri = new frmUrunSatisVerileri();
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            if (dgSatislar.RowCount > 0)
            {
                if (cbSecenek.SelectedIndex == 1)
                    ...
                    ExceleAktar(dtBaslangicTarihi.Value.ToShortDateString() + " - " +
                                dtBitisTarihi.Value.ToShortDateString() + " Tarihli");
