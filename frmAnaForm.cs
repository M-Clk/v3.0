    ...
    public partial class frmAnaForm : Form
    {
        ...
        private void btnStokIslemleri_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (nf_cagirdi) nf_cagirdi = false;
        ...
        {   if (dgSepet.RowCount > 0)
            {
                ...
                if (Sor == DialogResult.Yes)
                    Application.Exit();
                else { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
            }
        ...
        private void frmAnaForm_Load(object sender, EventArgs e)
        {
            ...
            login.ShowDialog();
            login.Dispose();
            if (!Program.giris) return;
            SonSatisGoruntule();
            while(cbMiktar.Items.Count<100)cbMiktar.Items.Add(cbMiktar.Items.Count+1);
        ...
        {
            if (e.KeyCode == Keys.Right) cbMiktar.Select();
        ...
        miktarDurum MiktarHesapla(string kod, decimal miktar, decimal istenenMiktar) //Sorgulanan ürünün miktarının yeterliliğini kontrol et ve sepette varsa seçilen miktarın eklemesi için yoksa sepete eklemesi için enum gönder
        {
            ...
            if (istenenMiktar > miktar) return miktarDurum.sepette_yok_yetersiz;//istenen miktar sepette yok ve yetersizse onun anlamına gelen enumu gönder
            else return miktarDurum.sepette_yok_yeterli; //bunlardan hiçbiri değilse tek enum kaldı onu gönder
        }
        ...
        void ToplamHesapla() //Toplam fiyat tutarını hesapla
        {
                ...
                for (int i = 0; i < dgSepet.Rows.Count; i++)
                {
                    ...
                    if (i % 2 == 0) dgSepet.Rows[i].DefaultCellStyle.BackColor = Color.LightPink;
                    else dgSepet.Rows[i].DefaultCellStyle.BackColor = Color.LavenderBlush;
                }
        ...
        {
            if (e.KeyCode == Keys.Down) mtxtTelefon.Select();
            if (e.KeyCode == Keys.F1) { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
        }

        private void mtxtTelefon_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Down) chckMusteriKaydet.Select();
            if (e.KeyCode == Keys.Up) txtMusteriAdi.Select();
            if (e.KeyCode == Keys.F1) { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
        }

        private void chckMusteriKaydet_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Up) mtxtTelefon.Select();
            if (e.KeyCode == Keys.F1) { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
            if (e.KeyCode == Keys.Enter) chckMusteriKaydet.Checked = !chckMusteriKaydet.Checked;
        }
...

            {
                ...
                for (var kolon = 0; kolon < dgSepet.ColumnCount; kolon++)
                {
                    ...
                    for (var satir = 0; satir < dgSepet.RowCount; satir++)
                    {
                        ...
                        else if (kolon == 3 || kolon == 4) ws1.Cells[satir + 3, kolon + 1].Value = Convert.ToDecimal(dgSepet.Rows[satir].Cells[kolon].Value);
                        else if(kolon<6)
...
SqlOperation.con.Dispose();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else MessageBox.Show("Yönetici olmadığınızdan gelir-gider işlemlerine giriş yapamazsınız. Lütfen işletme yöneticisiyle görüşün.", "Yetkisiz İşlem", MessageBoxButtons.OK, MessageBoxIcon.Error);
            txtBarkodOku.Text="";txtBarkodOku.Select();
        }
        ...
        {
            if (e.ColumnIndex == 4 && e.RowIndex>=0 ) dgSepet.Rows[e.RowIndex].Cells[e.ColumnIndex].Style.BackColor= Color.Red;
        }

        private void dgSepet_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 4 && e.RowIndex >= 0) dgSepet.Rows[e.RowIndex].Cells[e.ColumnIndex].Style.BackColor = dgSepet.Rows[e.RowIndex].Cells[0].Style.BackColor;
        }
        ...
        private void rbNakit_CheckedChanged(object sender, EventArgs e)
        {
            if (rbNakit.Checked)
            {
                ...
                cbOdenenNakit.Enabled = true; txtOdenenKredi.Enabled = false;
        ...
        private void rbKrediKarti_CheckedChanged(object sender, EventArgs e)
        {
            if (rbKrediKarti.Checked)
            {
                ...
                cbOdenenNakit.Enabled = false; txtOdenenKredi.Enabled = true;
                cbTaksit.Enabled = true;
                txtOdenenKredi.Focus();
                txtOdenenKredi.SelectAll();
            }
            else rbKrediKarti.BackColor = tableLayoutPanel26.BackColor = rbNakit.BackColor;
        }
        bool d = true;
        ...
        void ParaUstuHesapla()
        {
            ...
            if (paraUstu < 0) lblParaUstu.Text = (-paraUstu).ToString("F2") + " TL alacaklısınız.";
            else lblParaUstu.Text = " Para Üstü : " + paraUstu.ToString("F2") + " TL";
        ...
        private void cbOdenenNakit_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.F1) { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
        ...
        private void cbOdenenNakit_TextChanged(object sender, EventArgs e)
        {
            ...
            catch
            {
                ...
                {
                    if (Convert.ToInt32(item.ToString()) > toplam) { cbOdenenNakit.SelectedItem = item; break; }
                    else cbOdenenNakit.SelectedItem = item; ;
                }
                odenenNakit = Convert.ToDecimal(cbOdenenNakit.Text);
                cbOdenenNakit.Focus();
            }
        ...
        private void txtOdenenKredi_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Left) { rbNakit.Checked = true; cbOdenenNakit.Focus(); }
            else if (e.KeyCode == Keys.F1) { txtBarkodOku.Text = ""; txtBarkodOku.Select(); }
        }
            ...
            if (chckMusteriKaydet.Checked)
            {
                if (txtMusteriAdi.Text != "" && mtxtTelefon.Text != "")
                {
                    if (chkBorc.Visible && txtBorc.Text != "")
                    {
                        try
                        {
                            if (Convert.ToDecimal(txtBorc.Text) > 0) Borc = Convert.ToDecimal(txtBorc.Text);
                            else Borc = 0;
                        }
                        catch
                        {
