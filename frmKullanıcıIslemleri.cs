...
namespace Otomasyon
{
    public partial class frmKullanıcıIslemleri : Form
    {
        ...
        private string ilkKAdi = "";
        private readonly RegistryKey sifreIste = Registry.CurrentUser.CreateSubKey("SOFTWARE\\Otomasyon");
        private readonly DbOperations SqlOp = new DbOperations();
        private bool uyariVer = true;
        ...
        private void KullanicilariGuruntule()
        {
            ...
            var KullaniciOku = SqlOp.OkuProcedure("KULLANICILARIGETIR", new SqlParameter[0]);
            while (KullaniciOku.Read())
            {
                var rows = new string[dgKullanicilar.ColumnCount];
        ...
        private bool KarakterSayisiKontrol(TextBox txt, int sinir)
        {
            if (txt.TextLength >= sinir) return true;
            return false;
        }

        private bool KarakterKontrol(TextBox txt)
        {
        ...
        {
            if (cbSifreGoster2.Checked) txt_Sifre.PasswordChar = '\0';
        ...
        private void dgKullanicilar_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1 && e.ColumnIndex == 4)
            {
                ...
                {
                    MessageBox.Show(
                        "Kendinizi silemezsiniz. Ancak kullanıcı adınız dahil bilgilerinizi güncelleyebilirsiniz. Güncellemek için kullanıcı adınıza çift tıklayın. ",
                        "Yetkisiz İşlem Girişimi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                var prmtr = new SqlParameter[1];
        ...
        {
            if (e.ColumnIndex == 3 && e.Value != null) e.Value = new string('•', e.Value.ToString().Length);
        }
        ...
        private bool IlkKontroller()
        {
            if (!KarakterKontrol(txtK_Ad) || !KarakterKontrol(txt_Sifre))
            {
                MessageBox.Show("Geçersiz karakter kullanıldı.", "Karakter Girişi Hatası", MessageBoxButtons.OK,
            ...
            {
                MessageBox.Show("Şifre en az 6 karakter olmalıdır.", "Karakter Girişi Hatası", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                return false;
            }
            ...
            {
                MessageBox.Show("Kullanıcı adı en az 3 karakter olmalıdır.", "Karakter Girişi Hatası",
        ...
        private void btnKGuncelle_Click(object sender, EventArgs e)
        {
            ...
            {
                var soru = MessageBox.Show(
                    "Kendi kullanıcı bilgilerinizi değiştirmeye çalışıyorsunuz. Eğer yetkinizi değiştirdiyseniz çıkış yapılacak, artık kullanıcıları ve müşteri işlemlerini yönetemeyeceksiniz. Devam edilsin mi?",
                    "Kritik İşlem", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            {
                MessageBox.Show("Girdiğiniz kullanıcı adına sahip bir kullanıcı zaten var. Lütfen tekrar deneyin.",
                    "Kullanıcı Adı Çakışması", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (bayrak)
                {
                    ...
                    if (cbYetki.SelectedIndex == 0)
                    {
                        ...
                        MessageBox.Show(
                            "Yetkinizi değiştirdiniz. Artık yönetici değilsiniz. Başka kullanıcılara müdahale etme yetkiniz elinizden alındığı için kullanıcı işlemlerinden çıkılıyor.",
                            "Çıkış Bilgilendirme", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Close();
                        return;
                    }
                }
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            var dr = SqlOp.OkuProcedure("KULLANICISIFRESI", prmtr);
            if (!dr.Read())
            {
                MessageBox.Show("Eski şifrenizi yanlış girdiniz. Lütfen tekrar deneyin.", "Şifre Uyuşmazlığı Hatası",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
        ...
        private void chbSifreIste_CheckedChanged(object sender, EventArgs e)
        {
...

            {
                ...
                if (uyariVer)
                    MessageBox.Show(
                        "Artık şifre giriş ekranı açılmayacak. Ancak bunun pek güvenli bir seçenek olmadığını hatırlatmak isteriz. Tüm işlemler sizin kullanıcı adınız ile yapılacak. Siz ve sizin dışınızda yönetici yetkisine sahip herhangi bir kullanıcının bu seçeneği değiştirebileceğini unutmayın.",
                        "Güvenli Olmayan Seçenek Uyarısı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                uyariVer = false;
            }
            else
            {
